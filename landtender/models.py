...
class Lot:
    ...
    #: Код населённого пункта ЦСБ (``KodYeshuv``). Портал присылает его почти
    #: всегда, а текстовое имя — далеко не всегда: из 817 действующих лотов
    #: город текстом был известен ни у одного. Для сравнения участков имя и не
    #: нужно — нужен признак «тот же населённый пункт», и код им является
    #: надёжнее названия, у которого десяток написаний.
    ...
    #: Минимальная цена тендера (מחיר מינימום). Хранится всегда — и тогда,
    #: когда торги уже прошли и в ``price_nis`` лежит цена сделки. Пара
    #: «с чего начали / чем кончилось» по одному и тому же участку —
    #: единственный способ узнать, во сколько раз торги поднимают старт.
    #: По одной цене из двух этот вопрос не задать, а без ответа минимальную
    #: цену не с чем сравнивать: оценка построена на ценах сделок.
    reserve_price_nis: float | None = None
    #: Ожидаемая цена сделки для лота, у которого известна только
    #: минимальная: старт, поднятый на среднюю надбавку по архиву.
    #: Сравнивать с оценкой честно именно её, а не стартовую цену.
    expected_price_nis: float | None = None
