...
def _price_score(lot: Lot) -> float | None:
    ...
    Сравнивается ожидаемая цена сделки, а не объявленная. У действующего
    тендера объявлена минимальная цена, а оценка построена на ценах
    состоявшихся сделок; сравнение одного с другим давало «дешевле оценки»
    почти всем подряд — 127 лотов из 159 в одном из прогонов. Надбавку,
    которую торги добавляют к старту, считает
    :func:`valuation.reserve_premium` по архиву, и результат приходит сюда
    в ``expected_price_nis``. Пока архив её не даёт, поле пустое и
    сравнивается минимальная цена — с прежним завышением балла; об этом
    прямо сказано в сводке, а не оставлено на догадку читателя.
    """
    if not lot.estimate_nis or not lot.price_nis:
        return None
    ratio = (lot.expected_price_nis or lot.price_nis) / lot.estimate_nis
